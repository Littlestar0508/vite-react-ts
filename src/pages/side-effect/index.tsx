...
function SideEffectDemo() {
  // 관심사 별 묶음 - 상태, 파생된 상태, 상태 업데이트 로직 포함하는 이벤트 핸들러, 이펙트, 종속성 배열에 상태가 포함된 이벤트
  ...
  useEffect(() => {
    // 이벤트 핸들러
    // JSX에서 해당 이벤트 핸들러를 사용하는 것이 아니라, 이펙트 함수 내부에서 이벤트 구독과 취소가 이루어지므로 이 안에 존재하는 것이 적절
