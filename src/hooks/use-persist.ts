...
function usePersist<T>(
  ...
  // useCallback: 함수 값만 기억
  // useMemo를 사용해 함수 값을 기억할 수도 있지만, 함수 가독성 측면에서 함수 값인 경우 useCallback을 사용
  const saveToStorage2 = useCallback(
    (value: T) => setStorage(key, value, storage),
    [key, storage]
  );
  ...
  const removeInStorage2 = useCallback(
    () => removeStorage(key, storage),
    [key, storage]
  );
